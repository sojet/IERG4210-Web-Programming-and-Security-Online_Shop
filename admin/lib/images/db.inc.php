...
function ierg4210_prod_insert() {
    ...
    // $q->bindParam(3, $price, PDO::PARAM_FLOAT);
    // $q->bindParam(4, $description, PDO::PARAM_STR);
    // $q->bindParam(5, $filename, PDO::PARAM_STR);
    // $q->bindParam(6, $pid, PDO::PARAM_INT);
    $q->bindParam(1, $catid);
    $q->bindParam(2, $name);
    $q->bindParam(3, $price);
...
function ierg4210_prod_edit(){
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], "/var/www/html/admin/lib/images/" . $lastId . ".png")) {
        // redirect back to original page; you may comment it during debug
        $sql='UPDATE products SET filename = ? WHERE pid =? ;';
